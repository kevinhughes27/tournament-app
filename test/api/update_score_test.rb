...
class UpdateScoreTest < ApiTest
  ...
  test "can't update score if not safe (mock)" do
    ...
    input = {game_id: relay_id('Game', game.id), home_score: 14, away_score: 12}
  ...
  test "can't update score if not safe" do
    ...
    input = {game_id: relay_id('Game', game1.id), home_score: game1.away_score, away_score: game1.home_score}
  ...
  test "confirm unsafe update (mock)" do
    ...
    input = {game_id: relay_id('Game', game.id), home_score: 14, away_score: 12, confirm: true}
  ...
  test "confirm unsafe update" do
    ...
    input = {game_id: relay_id('Game', game1.id), home_score: new_home_score, away_score: new_away_score, confirm: true}
  ...
  test "can't submit a tie for a bracket game" do
    ...
    input = {game_id: relay_id('Game', game.id), home_score: 5, away_score: 5}
  ...
  test "confirms the game" do
    game = FactoryBot.create(:game)
    input = {game_id: relay_id('Game', game.id), home_score: 15, away_score: 11}
    execute_graphql("updateScore", "UpdateScoreInput", input, @output)
    assert_success
  ...
  test "resolves any disputes" do
    ...
    input = {game_id: relay_id('Game', game.id), home_score: 15, away_score: 11, resolve: true}
    execute_graphql("updateScore", "UpdateScoreInput", input, @output)
    assert_success
